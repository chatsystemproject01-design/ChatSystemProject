...
def send_async_email(app, msg):
    with app.app_context():
        try:
            print(f">>> [EMAIL PENDING] Bắt đầu gửi email đến {msg.recipients}...", flush=True)
            mail.send(msg)
            ...
            error_msg = f">>> [EMAIL ERROR] Lỗi gửi email: {str(e)}"
...
def send_otp_email(to_email, otp_code):
    """
    Gửi mã OTP xác thực qua email (Chạy ngầm để không gây lỗi request timeout)
    """
